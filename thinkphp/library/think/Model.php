...
abstract class Model implements \JsonSerializable, \ArrayAccess
{
    ...
    // 创建时间字段
    protected $create_tm = 'create_time';
    // 更新时间字段
    ...
    {
        if (is_null($value) && $this->autoWriteTimestamp && in_array($name, [$this->create_tm, $this->updateTime])) {
            // 自动写入的时间戳字段
    ...
    public function getAttr($name)
    {
        ...
        } elseif (in_array($name, [$this->create_tm, $this->updateTime])) {
    ...
    public function save($data = [], $where = [], $sequence = null)
    {
        ...
        if ($this->isUpdate) {
            ...
            // 自动写入创建时间
            if ($this->autoWriteTimestamp && $this->create_tm && (empty($this->change) || !in_array($this->create_tm, $this->change))) {
                $this->setAttr($this->create_tm, null);
            }
