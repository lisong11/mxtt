...
class ttyAdminValidate extends BaseValidate
{

    protected $needCityRoleId = 0;
    protected $rule           = array(
        'id'=>'',
        'tty_user_name' => 'require|checkUserNameMax',
        'password' => 'require',
        'real_name' => 'require',
        'role_id'  => 'require|number|gt:0|checkAdmin',
    );
    protected $message        = array(
        'tty_user_name.require' => '账号不能为空',
...
//        'real_name.unique' => '真实姓名已经存在',
    ...
    protected $scene          = array(
        "login" => array(
            "tty_user_name" => "require|loginCheckName",
            "password" => "require|loginCheckPassword"
        ),
        "add"   => array(
            'tty_user_name' => 'require|checkUserNameMax|checkNameNotEffectiveRepeat',
            'real_name' => 'require|checkRealNameMax',//|checkRealNameNotEffectiveRepeat
            'role_id',
        ),
        "edit"  => array(
            'tty_user_name' => 'require|checkUserNameMax|checkNameNotSelfNotRepeat',
    ...
    protected function checkEffectiveSelf($value, $rule, $data)
    {
        ...
        return $this->ttyAdminModel->where($where)->count() > 0 ? true : '人员不存在';
    }
...
//        }
    ...
    protected function checkRealNameNotEffectiveRepeat($value, $rule, $data)
    {
        ...
        return $this->checkValueNotEffectiveRepeat($value, $rule, $data, "ttyAdminModel", "real_name", "姓名已经存在", $baseWhere);
    }
    protected function checkNameNotEffectiveRepeat($value, $rule, $data)
    {
        ...
        return $this->checkValueNotEffectiveRepeat($value, $rule, $data, "ttyAdminModel", "tty_user_name", "账号已经存在", $baseWhere);
    }
    ...
    protected function checkRealNameNotSelfNotRepeat($value, $rule, $data)
    {
        ...
        return $this->checkValueWithOtherFields($value, $rule, $data, "ttyAdminModel", "real_name", "姓名已经存在", "id", 2, $baseWhere);
    }
    protected function checkNameNotSelfNotRepeat($value, $rule, $data)
    {
        ...
        return $this->checkValueWithOtherFields($value, $rule, $data, "ttyAdminModel", "tty_user_name", "账号已经存在", "id", 2, $baseWhere);
    }
...
//    }
    ...
    {
        $obj   = $this->ttyAdminModel;//\think\Loader::model("AdminList");
        $where = array(
            "tty_user_name" => $value
        );
        $field = array(
            "tty_user_list.*",
            "tty_role_list.is_delete role_is_delete",
            "tty_role_list.status role_status",
        );
        $res   = $obj
                ->field($field)
                ->where($where)
                ->join("tty_role_list", "tty_user_list.role_id = tty_role_list.id")
                ->find();
        if (empty($res)) {
            return "账号不存在";
        }
    ...
    {
        $obj = $this->ttyAdminModel;//\think\Loader::model("AdminList");
        $res = $obj->where(array("tty_user_name" => $data["tty_user_name"]))->find();
