...
class TtyNodeValidate extends BaseValidate
{

    // 设置当前模型对应的完整数据表名称
    protected $table  = 'tty_node_list';
    ...
    public function getRoleAllowNodeList($roleId)
    {
        ...
        $list  = $this
                ->field($field)
                ->join("tty_role_node_list", "tty_role_node_list.tty_node_id = tty_node_list.id")
                ->where($where)
                ->select();
    ...
    public function getAllRoleTreeNodeList($roleId, $where = array())
    {
        ...
        $field = array(
            ...
            "parent_node_id",
            "node_deep",
            "if(tty_role_node_list.id is null,0,1) AS is_allow");
        $list  = $this
                ->field($field)
                ->join("tty_role_node_list", "tty_role_node_list.tty_node_id = tty_node_list.id and tty_role_node_list.tty_role_id = ".$roleId,"left")
                ->where($where)
                ->order("order_by desc")
                ->select();
        $list  = $this->resetArrayKey($list);
